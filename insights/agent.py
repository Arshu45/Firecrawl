...
from insights.tools import (
    get_active_offers,
    get_competitor_market_recommendation,
    get_category_trends,
    get_recommendation,
    get_top_competitors,
)
...
def _extract_failed_tool_call(error_text: str) -> tuple[str, dict] | None:
    match = re.search(
    ...
    try:
        ...
        logger.warning("Failed to parse malformed tool call args | raw_args=%s", _truncate(raw_args))
        return None
...
- The get_recommendation tool returns structured strategy support, including situation, action, and why lines grounded in live data.

MULTI-COMPETITOR STRATEGY QUESTIONS (all competitors in one category):
- Trigger phrases: "our competitors", "all competitors", "competitors doing in", "what is the market doing and what can we do"
- Use: get_competitor_market_recommendation ONLY
- Do NOT chain multiple get_recommendation calls for the same category when the user asked about competitors in general
- Return one accumulated category recommendation, then support it with competitor comparisons
...
- For strategy responses, do not repeat generic fallback advice. Tailor the recommendation to the competitor's actual offer descriptions, promo mix, discount gaps, and the internal offer pattern.
...
- For strategy responses, include: urgency, recommended action, a concrete response range if justified by the evidence, why that tactic fits the competitor pattern, and 2 to 3 supporting facts.
- For multi-competitor strategy responses, compare 2 or more competitors in the same category, call out their common patterns, and give one combined response for {CLIENT_BRAND}.
- When the tool reports missing internal data or missing competitor data, be explicit about that limitation instead of inventing a strategy.
        ...
        try:
            ...
            for attempt in range(1, retry_attempts + 1):
                try:
                    ...
                    if "tool_use_failed" not in str(invoke_error):
                        raise
                    logger.warning(
                        "Agent invoke hit tool_use_failed | session_id=%s | attempt=%d/%d | error=%s",
                        session_id,
                        attempt,
                        retry_attempts,
                        _truncate(str(invoke_error)),
                    )
...
""")
            ...
            fallback_call = _extract_failed_tool_call(str(e))
            if fallback_call:
                ...
                if tool:
                    logger.warning(
                        "Using manual tool fallback after malformed Groq function call | session_id=%s | tool=%s | args=%s",
                        session_id,
                        tool_name,
                        _truncate(tool_args),
                    )
                    try:
                        ...
                        fallback_messages = [
                            (
                                "system",
                                (
                                    f"You are {CLIENT_BRAND}'s Lead Pricing Strategist and Market Intelligence Assistant. "
                                    "A tool has already been executed successfully. "
                                    "Do not call any tools. "
                                    "Answer the user's question using only the provided tool result. "
                                    "Be specific, use exact numbers, and keep the response concise but complete."
                                ),
                            ),
                            (
                                "human",
                        ...
                        logger.error(
                            "[%s] Manual tool fallback failed: %s",
                            session_id,
                            str(fallback_error),
                            exc_info=True,
                        )
...
#     known_categories_lower = {c.lower() for c in KNOWN_CATEGORIES}
#     has_context_entity = (
#         bool(words & known_categories_lower) or bool(words & KNOWN_BRANDS)
#     )
#     if has_context_entity:
#         return False
...
#         for i, c in enumerate(self.calls, start=1):
#             rows.append(
#                 f"    #{i:<7} {c['prompt']:>12} tkns  "
#                 f"{c['completion']:>14} tkns  {c['total']:>8} tkns"
#             )
#         grand = (
...
#             # Build competitor and category lists dynamically from settings.
#             # Adding a new competitor or category to settings.py is automatically
#             # reflected in the system prompt — no manual edits needed here.
#             competitor_list = ", ".join(COMPETITOR_SITES.keys())
#             category_list = ", ".join(KNOWN_CATEGORIES)
...
# OBSERVATION QUESTIONS (what is a competitor doing):
# - Trigger words: "what is X doing", "show me", "list", "what offers", "what are they running", "how is the market", "trends", "overview"
# - Use: get_category_trends OR get_active_offers
# - Do NOT call get_recommendation for observation questions
...
# - Trigger words: "what should we do", "how do we respond", "what can we do", "recommend", "strategy", "how do we compete", "what's our move", "respond to"
# - Use: get_recommendation FIRST, then stop
# - Do NOT call get_active_offers or get_category_trends unless the user also asked for examples in the same message

# LISTING QUESTIONS (show me specific offers):
# - Trigger words: "show me", "list", "give me examples", "concrete offers", "what are they running"
# - Use: get_active_offers ONLY
# - Your response MUST lead with the actual offer list — do not replace it with a recommendation summary
# - Do NOT call get_recommendation for listing questions

# OVERVIEW QUESTIONS (trends across all categories):
# - Trigger words: "category trends", "all categories", "overview", "what's happening", "for all"
# - Use: get_category_trends with no category filter
# - Call it ONCE and stop — do not follow up with get_active_offers or get_recommendation per category
# - One overview question = exactly one tool call
...
# We are "{CLIENT_BRAND}". Refer to internal data as {CLIENT_BRAND}'s promotions.
# Be specific. Use exact percentages and brand names.
# Do not add closing filler sentences like "this will help {CLIENT_BRAND} gain a competitive edge." Say nothing if you have nothing to add.
# Do not repeat the same point twice in different words.

# For STRATEGY responses, use this exact three-line structure — no more, no less:

# SITUATION: [One sentence. What the competitor is doing right now. Use exact numbers — their average discount, deepest discount, offer count. Compare directly to {CLIENT_BRAND}'s position.]
# ACTION: [One sentence. Exactly what {CLIENT_BRAND} should do. Name a specific offer type and discount range. State urgency — "this week" or "within 48 hours" for high urgency.]
# WHY: [One sentence. Why this tactic fits the competitor's promotion pattern. Reference their actual promo mix — bundles, flat discounts, percentage. No generic phrases.]

# Example of a correct strategy response:
# SITUATION: Myntra is running 4 footwear offers averaging 45% off with their deepest at 70%, putting them 14 points ahead of Westside's 31% average.
# ACTION: Launch a "Spend ₹1999, get 40% off footwear" offer this week — targeted by category, not sitewide.
# WHY: Myntra mixes flat and percentage discounts in footwear, so a spend-threshold response matches their value without requiring Westside to match their 70% headline number.

# For OBSERVATION responses, include:
# - Offer count and activity level
# - Average and deepest discount
# - Promo type mix when available
# - 2 to 4 concrete offer examples with discount values

# For LISTING responses:
# - Lead with the actual offer list
# - Follow with a single sentence summarising the discount range
# - Do not add a recommendation unless the user asked for one

# For OVERVIEW responses:
# - Summarise by category — one line per category
# - Include which competitor leads each category and at what average discount
# - Do not deep-dive into any single category unless asked

# If the user asks for more detail, examples, or rationale, give a richer answer rather than repeating the short summary."""
...
#             # Cap history to last MAX_HISTORY_TURNS turns to control token growth.
#             # Each "turn" is one user message + one assistant message = 2 items,
#             # so we slice at -(MAX_HISTORY_TURNS * 2).
#             messages = _history_to_messages(history)[-(MAX_HISTORY_TURNS * 2):]
#             messages.append({"role": "user", "content": query})
...
#                 session_id,
#                 [tool.name for tool in self.tools],
#                 len(messages) - 1,  # exclude the current query turn
#             )
...
# def _format_strategy_response(text: str) -> str:
#     """
#     Ensures SITUATION, ACTION, and WHY each appear on their own line.
#     Acts as a deterministic safety net for cases where the LLM returns
#     them space-separated on a single line despite the prompt instruction.
#     Also converts the three labels into clean bullet points.
#     """
#     for label in ["ACTION:", "WHY:"]:
#         text = text.replace(f" {label}", f"\n{label}")

#     # Convert labels to bullet points with bold markdown
#     text = text.replace("SITUATION:", "• **Situation:**")
...
# For STRATEGY responses, use this exact three-line structure — no more, no less.
# Each label MUST be on its own separate line. Never put SITUATION, ACTION, and WHY on the same line.

# SITUATION: [One sentence. What the competitor is doing right now. Use exact numbers — their average discount, deepest discount, offer count. Compare directly to {CLIENT_BRAND}'s position.]
# ACTION: [One sentence. Exactly what {CLIENT_BRAND} should do. Name a specific offer type and discount range. State urgency — "this week" or "within 48 hours" for high urgency.]
# WHY: [One sentence. Why this tactic fits the competitor's promotion pattern. Reference their actual promo mix — bundles, flat discounts, percentage. No generic phrases.]

# Example of a correct strategy response (note each label is on its own line):
# SITUATION: Myntra is running 4 footwear offers averaging 45% off with their deepest at 70%, putting them 14 points ahead of Westside's 31% average.
# ACTION: Launch a "Spend ₹1999, get 40% off footwear" offer this week — targeted by category, not sitewide.
# WHY: Myntra mixes flat and percentage discounts in footwear, so a spend-threshold response matches their value without requiring Westside to match their 70% headline number.
...
#             final_message = result["messages"][-1].content
#             final_message = _format_strategy_response(final_message)
